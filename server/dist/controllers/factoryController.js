...
const getOne = (Model, populateOptions) =>
    (0, catchAsync_1.default)(async (req, res) => {
        ...
        if (data == null) throw new AppError(`${Model} is not defined`)
        res.status(200).json({
            status: 'success',
            data: {
                data,
...
const updateOne = (Model, unallowedFields) =>
    (0, catchAsync_1.default)(async (req, res) => {
        const content = { ...req.body }
        const contentKeys = Object.keys(content)
        if (unallowedFields && unallowedFields.some((field) => contentKeys.includes(field))) {
            throw new AppError(`Fields of ${unallowedFields.join(', ')} cannot be updated`)
        }
        // @ts-ignore
        const { id } = req.params
        ...
        if (updatedData == null) throw new AppError(`Failed to update ${Model}`)
        res.status(200).json({
            status: 'success',
            data: {
                data: updatedData,
