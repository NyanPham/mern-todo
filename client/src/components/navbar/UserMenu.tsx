...
const UserMenu = () => {
    ...
    return (
      <div className="relative">
			...
			{currentUser == null ? (
				<>
					<div className={`w-max transition duration-300 absolute top-[120%] right-0 bg-white shadow-md rounded-lg overflow-hidden ${isOpen ? 'translate-y-0 opacity-100' : '-translate-y-2 opacity-0'}`}>
				...
				<>
					<div className={`w-max transition duration-300 absolute top-[120%] right-0 bg-white shadow-md rounded-lg overflow-hidden ${isOpen ? 'translate-y-0 opacity-100' : '-translate-y-2 opacity-0'}`}>
						<div className="py-2 px-5 cursor-pointer hover:bg-sky-500/70" onClick={() => {}}>My tasks</div>
						<hr />			
						<div className="py-2 px-5 cursor-pointer hover:bg-sky-500/70" onClick={() => {}}>Create tasks</div>
						<hr />			
						<div className="py-2 px-5 cursor-pointer hover:bg-sky-500/70" onClick={handleLogOut}>Logout</div>
					</div>  
				</>
			)}
      </div>
